...
use crate::{clientes::cliente_model::ClienteNf, financeiro::{email_service::{send_nf, setup_email}, nfs::save_nf, nfs_model::NfDto}};

use super::{email_service::send_nf_lote, nfs::save_nf_lote, nfs_model::NfLoteDto};
...
pub async fn exporta_nfs(pool: &PgPool,mailer:&AsyncSmtpTransport<Tokio1Executor> ) -> Result<(),anyhow::Error> {
    ...
    if let Some((last_file_path, _)) = last_modified_file.clone() {
        ...
        let nf_lote = NfLoteDto {
            path: last_file_path_str,
            month: month as i32,
            year: year,
        };

        save_nf_lote(&pool, &nf_lote).await.context("Failed to save nf lote")?;
    } else {
...
pub async fn gera_nfs(pool:&PgPool,cliente:&ClienteNf,value:f32,mailer: Option<AsyncSmtpTransport<Tokio1Executor>>,payment_id: i32) -> Result<(),anyhow::Error> {
    ...
    if let Some(mailer) = mailer {
        ...
        save_nf(&pool, &nf).await.context("Erro ao salvar nota fiscal no banco de dados")?;
    } else {
